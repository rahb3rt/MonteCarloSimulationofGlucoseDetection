...
int main ()
{
	...
	while (1) {
	  ...
	  for(int i=0; i<N2; i++) {
	    ...
	    if (ph->nsct == 1) {
		// the extra factor e/sqrt(csca): e for escaping probability, and 1/sqrt(csca) scales Eout to be the electric field over the unit solid angle in sn direction
		...
		// the extra factor e1/sqrt(csca): e1 for escaping probability, and 1/sqrt(csca) scales EoutRev to be the electric field over the unit solid angle in sn direction
		EoutRev[0] = S2_R1*(cosphi1prime*tmp1 + sinphi1prime*tmp2);
		EoutRev[1] = S1_R1*(-sinphi1prime*tmp1 + cosphi1prime*tmp2);

		AoutRev = sqrt(norm(EoutRev[0]) + norm(EoutRev[1]));
		EoutRev[0] /= AoutRev;
		EoutRev[1] /= AoutRev;

		AoutRev *= e1/sqrt(csca);
