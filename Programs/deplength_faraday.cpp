...
const double verdet[3]={0.5,0,0};        // angle of Faraday rotation per one lsa, = VB/2 cdot displacement, the factor 2 comes from right and left-circular polarization rotates by +-angle respectively.
...
void FaradayRotation(const double dx, const double dy, const double dz, double* cosphi, double* sinphi)
{
    ...
    double delta = d*alpha + verdet[0]*dx + verdet[1]*dy + verdet[2]*dz;
    double tmp;
    //fprintf(stderr, "%f\n", delta);
...
int main ()
{
	...
	while (1) {
	  ...
	  for(int i=0; i<N2; i++) {
	    ...
	    FaradayRotation(ph->x - ph->xold, ph->y - ph->yold, ph->z - ph->zold, &cosphiNminusOne, &sinphiNminusOne);
	    axb(nn, sn, mn);
...
#else
		dcmplx tmp1, tmp2;
		tmp1 = TRev[0][0]*(cosphinprime*EoutRev[0] + sinphinprime*EoutRev[1]) + TRev[0][1]*(-sinphinprime*EoutRev[0] + cosphinprime*EoutRev[1]);
 		tmp2 = TRev[1][0]*(cosphinprime*EoutRev[0] + sinphinprime*EoutRev[1]) + TRev[1][1]*(-sinphinprime*EoutRev[0] + cosphinprime*EoutRev[1]);
#endif
